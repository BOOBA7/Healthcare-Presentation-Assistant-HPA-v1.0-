...
def test_hcp_can_upload_validate_analyze_and_generate_blueprint_in_browser(tmp_path, monkeypatch):
    ...
    with sync_playwright() as playwright:
        ...
        with _serve_fastapi_app() as base_url:
            ...
            expect(page.locator("#prototype-declaration-required")).to_be_hidden()
            page.locator(".workspace-tab[data-workspace='resources']").click()
            page.locator("#pdf-file").set_input_files(str(pdf_path))
            expect(page.locator("#prototype-declaration-required")).to_be_visible()
            page.locator("#prototype-declaration-required [data-prototype-declaration]").select_option("synthetic")
            page.locator("#prototype-declaration-required [data-prototype-acknowledged]").check()
            page.locator('#prototype-declaration-required [data-action="declare-prototype"]').click()
            expect(page.locator("#prototype-declaration-required")).to_be_hidden()
            expect(page.locator("#project-declaration-status")).to_contain_text("synthetic")
            expect(page.locator("#evidence-context-mode")).to_have_count(0)
            page.locator(".workspace-tab[data-workspace='presentation']").click()

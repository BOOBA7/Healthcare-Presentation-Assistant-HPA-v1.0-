...
function evidenceDetails(references = []) { return references.length ? `<div class="evidence"><strong>Preuves vérifiées</strong>${references.map(reference => `<p><span>${escapeHtml(reference.title || "Source")}</span> · ID ${escapeHtml(reference.resource_id)} · p. ${escapeHtml(reference.page)}<br><q>${escapeHtml(reference.evidence_excerpt)}</q></p>`).join("")}</div>` : ""; }
